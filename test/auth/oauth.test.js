...
it('should complete first-time login and create a new user', async () => {
    ...
    const createNewUserProfile = sinon.stub(UserProfile, 'create').resolves(userProfile)
    ...
    expect(UserProfile.create.calledWithExactly({ displayName: 'Mr oAuth' })).to.be.true
  ...
  it('should update the last login date of an existing user, and any new data from the oAuth provider if new data exists', async() => {
    ...
    const findUserAndUpdateStub = sinon.stub(User, 'findOneAndUpdate').resolves(updatedUser) // updates existing user
    const findProfileAndUpdateStub = sinon.stub(UserProfile, 'findOneAndUpdate').resolves(updatedUserProfile) // updates existing profile
    ...
    expect(findUserAndUpdateStub.calledWithMatch(
            { googleId: 'profile-id' },
                {
                    $set: {
                        name: {
    ...
    const returnedUpdatedUserProfileValues = findProfileAndUpdateStub.getCall(0).args[1]
    expect(returnedUpdatedUserProfileValues.$set.displayName).to.equal('newGivenName')
    expect(returnedUpdatedUserProfileValues.$set.lastLogin.getTime()).to.equal(fakeDate.getTime())

    // data passed to User.findOneAndUpdate
    expect(findProfileAndUpdateStub.calledWithMatch(
            { _id: 'userprofile-id' },
            {
              $set: {
                displayName: 'newGivenName',
